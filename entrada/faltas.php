...
if (mysqli_num_rows($result_alunos) > 0) {
    ...
    while ($aluno = mysqli_fetch_assoc($result_alunos)) {
        ...
        $status = 'Faltante'; // Define o status como faltante por padrão
        $classStatus = 'faltante'; // Classe CSS para o status faltante
        ...
        // Verifica se o aluno faltou (não tem registros de entrada nem de atraso)
        if ($stmt_entrada->num_rows == 0 && $stmt_atraso->num_rows == 0) {
            // Insere o registro de falta na tabela registro_de_faltas se não existir
            $sql_falta_insert = "INSERT INTO registro_de_faltas (matricula, nome, turma, status_do_aluno, data_falta, hora_registro_falta) 
                SELECT ?, ?, ?, ?, ?, ? 
                FROM DUAL 
                WHERE NOT EXISTS (SELECT * FROM registro_de_faltas WHERE matricula = ? AND data_falta = ?)";
            
            // Prepara a consulta de inserção
            $stmt_falta_insert = $conn->prepare($sql_falta_insert);
            
            // Corrige o número de parâmetros para corresponder à consulta
            $stmt_falta_insert->bind_param("ssssssss", $matricula, $nome, $turma, $status, $data_hoje, $hora_atual, $matricula, $data_hoje);
            
            // Executa a inserção
            $stmt_falta_insert->execute();
            
            // Fecha o statement de inserção
            $stmt_falta_insert->close();
        }
    ...
    <head>
        ...
        <title>Registro de Presenças hoje <?php echo $data_hoje ?></title>
